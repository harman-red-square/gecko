...
/**
 * IPC works on remote iframe. Remote iframe is separated by processes, so
 * called parent process and content process.
 * It delivers messages between processes to find next focusable element over
 * process boundary.
 */
...
function IPC_searchFocusOnChild(aIFrame, aDirection, aStartRect) {
  ...
  aIFrame.QueryInterface(Ci.nsIFrameLoaderOwner)
         ...
         .sendAsyncMessage(SearchMessage.sendToChild,
                           {direction: aDirection.toJSON(), rect: aStartRect});
}
