...
/**
 * Navigator is core of SpatialNavigationB2G.
 * It finds next focusable element and give focus to found one.
 * While finding and moving focus, it emits event via Addon.
 * See Addon.jsm for more detail.
 */
...
this.Navigator = {
  /**
   * Determine aKey have to be forwarded into nested iframe
   * @param {Integer} aKey
   */
  ...
  moveFocusFromParent(aTopWindow, aRectFrom, aDirection) {
    ...
    // By setting parameter aWithinProcessBoundary true, do not request parent
    // process to find focusable element when searching focusable element is
    // triggered by parent process via IPC message and there's nothing to focus.
    // See https://bugzil.la/1114536#c36 for some more detail
